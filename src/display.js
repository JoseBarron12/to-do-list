...
const displaySectionBtns = (parent, pageName) => {
  ...
  if (pageName != "all") {
    ...
    page[pageName].forEach((btn) => {
      ...
      contentbtn.textContent = displayStringBefore(toUpperCaseFirstChar(btn))
        ? strBfr + toUpperCaseFirstChar(btn)
        : toUpperCaseFirstChar(btn);
...
export const display = (function () {
  ...
  const navbar = () => {
    ...
    buttonTypes.forEach((buttonType) => {
      ...
      const numOfImpTasks = AllTasks.getNumOfImportantTasks(
        allTasksOfUser.getTasksFromName(buttonType),
      );
  ...
  const menuButtonSection = (navbarOpened) => {
    ...
    menuTab.style.backgroundColor = navbarOpened
      ? "var(--secondary)"
      : "var(--primary)";
  };
  const dropDownMenu = (parent, isOpen) => {
    ...
    const menuDown = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "svg",
    );
    ...
    const titleDown = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "title",
    );
    titleDown.textContent = "menu-down";

    const pathDown = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "path",
    );
    ...
    // Create "menu-up" SVG
    const menuUp = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "svg",
    );
    ...
    const titleUp = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "title",
    );
    titleUp.textContent = "menu-up";

    const pathUp = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "path",
    );
    ...
    if (isAddTaskWin) {
      ...
      if (currTask.projects.length != 0) {
        const option = select.querySelector(
          "option[value='" + `${currTask.projects[0].getId()}` + "']",
        );
  ...
  const mainPage = (name) => {
    ...
    if (name == "all") {
      functionality.updateTaskDisplayBtn(
        mainPage.querySelectorAll("div.content-btns>button"),
        tasks,
        allTasksOfUser.getTasksFromName(name),
      );
    } else {
      functionality.updateHeaderDisplayBtn(
        mainPage.querySelectorAll("div.content-btns>button"),
        tasksSection,
      );
    }
  ...
  const listPage = (id) => {
    ...
    functionality.updateTaskDisplayBtnForProjects(
      mainPage.querySelectorAll("div.content-btns>button"),
      tasks,
      allTasksOfUser.getTasksOfProject(id),
      id,
    );
  };
  ...
  const displaySectionHeader = (
    ...
    const headerText = displayStringBefore(headerCurrentName)
      ? strBfr + headerCurrentName
      : headerCurrentName;
    ...
    if (isProject !== true) {
      const tasksInHdr = AllTasks.getTaskFromHdrName(
        allTasksOfUser.getTasksFromName(pageName),
        headerName,
      );
